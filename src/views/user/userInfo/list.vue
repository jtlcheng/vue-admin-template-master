      ...
      <el-table-column label="操作" width="200" align="center">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.status==1" type="success" size="mini"
                     icon="el-icon-star-off" @click="updateStatus(scope.row.id,0)">锁定</el-button>
          <el-button v-if="scope.row.status==0" type="warn" size="mini"
                     icon="el-icon-star-off" @click="updateStatus(scope.row.id,1)">取消锁定</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    fetchData(page = 1) {
      ...
      // 异步获取远程数据（ajax）
      this.page = page
      userInfo.getListByCondition(this.page, this.limit, this.searchObj).then(
        response => {
    ...
    updateStatus(id, status) {
      ...
      type: 'warning'
        ...
        //点击确定 远程调用ajax
        return userInfo.lockStatus(id,status)
      }).then(response=>{
        this.total % this.limit == 1 ? this.fetchData(this.page - 1) : this.fetchData(this.page)
        if (response.code){
          this.$message({
            type: 'success',
            message: '操作成功!'
